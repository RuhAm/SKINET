...
sub = "CEU_sweep_" if class_label == 1 else "CEU_neut_"
cls_folder = 'Sweep' if class_label==1 else 'Neutral'
...
for i in range(num_):
    ...
    #mode_full=np.array(mode_full)
    Mode_sliced = mode_full.mode[0]	
    #Mode_sliced=list(mode_full[0][0])
    upd_df = np.matrix(df)
 #if isinstance(Mode_sliced, np.float64):
    Mode_sliced = np.array([Mode_sliced])
