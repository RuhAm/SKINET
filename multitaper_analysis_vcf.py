...
spec_tensor_test = np.empty((TEST.shape[0], 36864))
...
for i in range(All_classes_stats_test["Stat_pi"].shape[0]):
  ...
  for j in range(9):
    ...
    coefs_resized= np.array(cv2.resize(coefs, (64, 64), interpolation=cv2.INTER_LINEAR))
    coefs_flattened = coefs_resized.flatten()
    spec_tensor_test[i][4096*j : 4096*j +4096] = coefs_flattened
  print(i)
...
np.save("TFA/"+ ss_file[2:] +"_multitaper", spec_tensor_test)
